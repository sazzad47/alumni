...
const Form: React.FC< {
  ...
  const handleSubmit = async (e: React.SyntheticEvent) => {
    ...
    const res = await patchData(
      `page/${data._id}`,
      { ...pageData, photo: media ? media[0] : "" },
      auth?.token
    );

    setUpdatedData(res.page);   
  ...
  const [photoURL, setPhotoURL] = useState<null | string>(data.photo);
    ...
    if (e.target.files) {
      ...
      if (newPhoto) {
        if (newPhoto.size > 10485760) {
          ...
          setPageData({ ...pageData, photo: newPhoto });
        ...
        >
          <Grid container spacing={2}>
            ...
            <Grid item xs={12}>
              ...
              <TextEditor detailedPage={detailedPage} setData={setPageData} />
            </Grid>
          </Grid>
          <Button
