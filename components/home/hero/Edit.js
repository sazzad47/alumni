...
const UpdateDialog = ({ item, setData, handleClose }) => {
  ...
  const { auth } = state;
  const initState = { file: "", caption: "", addToGallery: false };
    ...
    <form className="w-full pt-5">
      <Grid className="w-full flex flex-col md:items-center gap-2">
        ...
        <Grid className="w-full flex items-center justify-start">
          <Checkbox
            onChange={(e) => setData({ ...data, addToGallery: e.target.checked })}
            checked={data.addToGallery}
            sx={{
              color:
