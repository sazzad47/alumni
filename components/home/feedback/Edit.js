...
const Edit = ({ item, setData }) => {
  ...
  return (
    <Grid className="text-white">
      <Button onClick={handleOpenDialog} className='text-slate-200 bg-green-700 hover:bg-green-800 dark:bg-stone-500 dark:hover:bg-stone-600' startIcon={<EditIcon/>} variant="contained">Update</Button>
      <Dialog
        sx={{
          "& .MuiDialog-paper": {
            backgroundColor:
        ...
        open={open}
      >
        <UpdateDialog item={item} setData={setData} handleClose={handleCloseDialog} />
      </Dialog>
    </Grid>
  );
};
...
const UpdateDialog = ({ item, setData, handleClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    const res = await patchData(
      ...
      { ...userData, avatar: userData.avatar ? media[0] : "" },
      auth.token
    );
    setLoading(false);
...
const Form = ({
  ...
  const [photoURL, setPhotoURL] = useState(data.avatar);
