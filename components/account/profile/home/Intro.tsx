...
const PlaceOfBirthInfo = () => {
  ...
  const handleSubmit = async () => {
    ...
    const res = await patchData("user", { placeOfBirth }, auth?.token);
  ...
  useMemo(() => {
    if (!focused && !inputForm && !prevData) {
      setErrorMessage(["Please add your place of birth."]);
    } else if (focused) {
      setErrorMessage([]);
    }
...
const CurrentLocationInfo = () => {
  const { state, dispatch } = useContext(Context) as StoreProps;
  const { auth, loading } = state;
  const prevData = auth?.user?.currentLocation;
  const [inputForm, setInputForm] = useState<boolean>(false);
  ...
  const handleSubmit = async () => {
    ...
    const res = await patchData("user", { currentLocation }, auth?.token);
    dispatch({ type: GlobalTypes.LOADING, payload: false });
    if (res.err) return setErrorMessage([res.err]);
    dispatch({
      type: GlobalTypes.AUTH,
  ...
  useMemo(() => {
    if (!focused && !inputForm && !prevData) {
      setErrorMessage(["Please add your current location."]);
    } else if (focused) {
      setErrorMessage([]);
    }
  }, [focused, inputForm, prevData]);
